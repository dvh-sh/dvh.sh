    ...
    <div className="prose lg:prose-xl text-subtext0">
      <ReactMarkdown
        components={{
          ...
          p: ({ node, ...props }) => (
            <p className="mb-4 text-subtext0" {...props} />
          ),
          a: ({ node, ...props }) => (
          ...
          code: ({ node, inline, ...props }) =>
            inline ? (
              <code
                className="bg-surface0 text-pink px-1 py-0.5 rounded"
                {...props}
              />
            ) : (
              <code
                className="block bg-surface0 text-text p-4 rounded-lg mb-4 overflow-x-auto"
                {...props}
              />
            ),
        }}
